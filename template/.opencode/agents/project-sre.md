...
1. `AGENTS.md` — the full system map: what subsystems exist, who owns what,
   what the permission gates are
2. For **each subsystem** listed in `AGENTS.md`:
...
- Read subsystem documentation to understand what is expected behaviour
...
grows with it. You are as capable as the documentation you read — keep
the orchestrator honest about `SUMMARY.md` and `function_signature.md` being
up to date, because stale docs are your blind spots.
...
The orchestrator will route the fix to the appropriate role agent.
